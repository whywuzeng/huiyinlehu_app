...
public class MobileSecurePayer {
	...
	private ServiceConnection mAlixPayConnection = new ServiceConnection() {
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			synchronized (lock) 
			{
				...
				lock.notify();
				Log.i("x", "执行了Connect==33333------"+(null == mAlixPay));
			}
		}
	...
	public boolean pay(final String strOrderInfo, final Handler callback,
		...
		if (mAlixPay == null) {
			...
			Log.i("x", "mAlixPayConnection == null "+(null == mAlixPayConnection));
			Log.i("x", "mAlixPay===是否为空=111111=  "+(null == mAlixPay));
		}
		
		Log.i("x", "mActivity===是否为空==  "+(null == mActivity));
		Log.i("x", "mAlixPay===是否为空=22222=  "+(null == mAlixPay));
		// else ok.
