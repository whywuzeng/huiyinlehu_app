...
public class ImageViewTouch extends ImageView {
	...
	private void centerCharge(boolean horizontal, boolean vertical, boolean hasAni) {
		if (mBitmapDisplayed.getBitmap() == null) {
			return;
		}
	...
	public void centerWithAni(boolean horizontal, boolean vertical) {
	...
	// Setup the base matrix so that the image is centered and scaled
	// properly.
	private void getProperBaseMatrix(RotateBitmap bitmap, Matrix matrix) {
		...
		// We limit up-scaling to 3x otherwise the result may look bad
		// if it's
		// a small icon.
		...
		// } else {
		// matrix.postTranslate((viewWidth - w) / 2F, (viewHeight - h) /
		// 2F);
		// }
	}
	...
	public Matrix getImageViewMatrix() {
		// The final matrix is computed as the concatentation of the
		// base matrix
	...
	// Sets the maximum zoom, which is a scale relative to the base matrix.
	// It
	// is calculated to show the image at 400% zoom regardless of screen or
	// image orientation. If in the future we decode the full 3 megapixel
	// image,
	// rather than the current 1024x768, this should be changed down to
	// 200%.
	...
	public void zoomToNoCenterWithAni(float scale, float toScale, float centerX, float centerY) {
		ScaleAnimation animation = new ScaleAnimation(scale, toScale, scale, toScale, centerX, centerY);
		...
		// float oldScale = getScale();
		// float deltaScale = scale / oldScale;
		// mSuppMatrix.postScale(deltaScale, deltaScale, centerX,
		// centerY);
		// Animation animation = new MatrixTransformAnimation(
	...
	public void zoomIn(float rate) {
		if (getScale() >= mMaxZoom) {
			return; // Don't let the user zoom into the molecular
			        // level.
		}
		if (mBitmapDisplayed.getBitmap() == null) {
			return;
	...
	public void zoomTurnLeft() {
		...
		// 创建新的图片
		Bitmap resizedBitmap = Bitmap.createBitmap(mBitmapDisplayed.getBitmap(), 0, 0, mBitmapDisplayed.getWidth(), mBitmapDisplayed.getHeight(), matrix, true);
		setImageBitmapResetBase(resizedBitmap, true);
	}

	public void zoomTurnRight() {
		if (mBitmapDisplayed.getBitmap() == null) {
			return;
		}
		// 创建操作图片用的matrix对象
