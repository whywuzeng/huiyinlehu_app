...
public class GestureDetector {
	...
	/**
	 * The listener that is used to notify when gestures occur. If you want
	 * to listen for all the different gestures then implement this
	 * interface. If you only want to listen for a subset it might be easier
	 * to extend {@link SimpleOnGestureListener}.
	 */
	public interface OnGestureListener {
		 ...
		 * every down event. All other events should be preceded by
		 * this.
		 * 
		 * @param e
		...
		/**
		 * The user has performed a down {@link MotionEvent} and not
		 * performed a move or up yet. This event is commonly used to
		 ...
		 * 
		 * @param e
		 *                The up motion event that completed the first
		 *                tap
		 ...
		 * {@link MotionEvent} and the current move {@link MotionEvent}.
		 * The distance in x and y is also supplied for convenience.
		 * 
		 * @param e1
		 *                The first down motion event that started the
		 *                scrolling.
		 * @param e2
		 *                The move motion event that triggered the
		 *                current onScroll.
		 * @param distanceX
		 *                The distance along the X axis that has been
		 *                scrolled since the last call to onScroll. This
		 *                is NOT the distance between {@code e1} and
		 *                {@code e2}.
		 * @param distanceY
		 *                The distance along the Y axis that has been
		 *                scrolled since the last call to onScroll. This
		 *                is NOT the distance between {@code e1} and
		 *                {@code e2}.
		 * @return true if the event is consumed, else false
		 ...
		 * 
		 * @param e
		 *                The initial on down motion event that started
		 *                the longpress.
		 */
		void onLongPress(MotionEvent e);

		/**
		 * Notified of a fling event when it occurs with the initial on
		 * down {@link MotionEvent} and the matching up
		 * {@link MotionEvent}. The calculated velocity is supplied
		 ...
		 *                current onFling.
		 * @param velocityX
		 *                The velocity of this fling measured in pixels
	...
	public interface OnDoubleTapListener {
		 ...
		 * this will only be called after the detector is confident that
		 * the user's first tap is not followed by a second tap leading
		 ...
		 * 
		 * @param e
		 *                The down motion event of the first tap of the
		 *                double-tap.
		 * @return true if the event is consumed, else false
		 */
		 ...
		 * 
		 * @param e
		 *                The motion event that occurred during the
		 *                double-tap gesture.
		 * @return true if the event is consumed, else false
		 */
	...
	private class GestureHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case TAP:
				// If the user's finger is still down, do not
				// count it as a tap
	...
	/**
	 * Creates a GestureDetector with the supplied listener. This variant of
	 * the constructor should be used from a non-UI thread (as it allows
	 ...
	 *                 if either {@code listener} or {@code handler} is
	 *                 null.
	 * 
	 * @deprecated Use
	...
	/**
	 * Creates a GestureDetector with the supplied listener. You may only
	...
	private void init(Context context, boolean ignoreMultitouch) {
		...
		if (context == null) {
			...
			// largeTouchSlop =
			// configuration.getScaledLargeTouchSlop();
			largeTouchSlop = 18;
	 ...
	 *                the listener invoked for all the callbacks, or null to
	 *                stop listening for double-tap gestures.
	 */
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			if (mDoubleTapListener != null) {
				...
				if ((mCurrentDownEvent != null) && (mPreviousUpEvent != null) && hadTapMessage && isConsideredDoubleTap(mCurrentDownEvent, mPreviousUpEvent, ev)) {
					...
					// Give a callback with the first tap of
					// the double-tap
					handled |= mDoubleTapListener.onDoubleTap(mCurrentDownEvent);
					// Give a callback with down event of
					// the double-tap
		...
		case MotionEvent.ACTION_UP:
			...
			// Hold the event we obtained above - listeners may have
			// changed the
			// original.
